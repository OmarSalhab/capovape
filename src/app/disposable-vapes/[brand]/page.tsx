...
export async function generateMetadata({ params }: { params: Promise<{ brand: string }> }): Promise<Metadata> {
  ...
  const title = `${label} Disposable Vapes in Canada`;
  const description = `Shop ${label} disposable vapes at CapoVape Canada.`;
  const url = `${SITE}/disposable-vapes/${encodeURIComponent(brand)}`;
  return {
    title,
    description,
...
export default function DisposableBrandPage({ params }: { params: Promise<{ brand: string }> }) {
  ...
  return (
    <section className="py-12">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <BrandVapeGrid brand={brand} category="disposable" />
      </div>
    </section>
  );
}
