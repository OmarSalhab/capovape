...
export async function generateMetadata({
	...
	const title = `${label} Devices in Canada`;
	const description = `Shop ${label} devices at CapoVape Canada.`;
	const url = `${SITE}/pods-devices/devices/${encodeURIComponent(brand)}`;
	return {
		title,
		description,
...
export default async function DevicesBrandPage({
	params,
}: {
	params: Promise<{ brand: string }>;
}) {
	...
	const res = await fetch(
		`${baseUrl}/api/products/devices/${encodeURIComponent(brand)}`,
		{ cache: "force-cache" }
	);
